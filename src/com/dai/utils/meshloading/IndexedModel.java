...
package com.dai.utils.meshloading;
...
public class IndexedModel {
	...
	// 是否包含纹理
	private boolean containTexCoord;
	...
	public IndexedModel() {
		...
		containTexCoord = false;
	}

	public boolean isContainTexCoord() {
		return containTexCoord;
	}

	public void setContainTexCoord(boolean containTexCoord) {
		this.containTexCoord = containTexCoord;
	}
	...
	public void CalcNormals() {
		for (int i = 0; i < m_indices.size(); i += 3) {
			...
			Vector3f normal = v1.cross(v2).normailze();
		...
		for (int i = 0; i < m_normals.size(); i++)
			m_normals.get(i).set(m_normals.get(i).normailze());
	}
	...
	//
	// float deltaU1 = m_texCoords.get(i1).GetX() -
	// m_texCoords.get(i0).GetX();
	// float deltaV1 = m_texCoords.get(i1).GetY() -
	// m_texCoords.get(i0).GetY();
	// float deltaU2 = m_texCoords.get(i2).GetX() -
	// m_texCoords.get(i0).GetX();
	// float deltaV2 = m_texCoords.get(i2).GetY() -
	// m_texCoords.get(i0).GetY();
	//
