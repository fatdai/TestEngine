		...
		try {
			...
			while ((line = meshReader.readLine()) != null) {
				...
				if (tokens.length == 0 || tokens[0].equals("#"))
					...
					m_texCoords.add(new Vector2f(Float.valueOf(tokens[1]), 1.0f - Float
							.valueOf(tokens[2])));
				} else if (tokens[0].equals("vn")) {
					m_normals.add(new Vector3f(Float.valueOf(tokens[1]), Float.valueOf(tokens[2]),
	...
	public IndexedModel ToIndexedModel() {
		...
		for (int i = 0; i < m_indices.size(); i++) {
			...
			if (normalModelIndex == null) {
				...
				// normalModel.GetTangents().add(new Vector3f(0,
				// 0, 0));
			}
